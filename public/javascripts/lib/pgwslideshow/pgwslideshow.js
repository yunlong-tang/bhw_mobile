    ...
    $.fn.pgwSlideshow = function(options) {
        ...
        var updateHeight = function(height, animate) {
            ...
            if (typeof pgwSlideshow.plugin.find('.ps-current').animate == 'function') {
                pgwSlideshow.plugin.find('.ps-current').animate({
                    height: height
            ...
            if (pgwSlideshow.slideCount > 1) {
                ...
                if (pgwSlideshow.config.touchControls) {
                    pgwSlideshow.plugin.find('.ps-current').on('touchstart', function(e) {
                        try {
                            if (e.touches[0].clientX && pgwSlideshow.touchFirstPosition == null) {
                                pgwSlideshow.touchFirstPosition = e.touches[0].clientX;
                            }
                        } catch(e) {
                            pgwSlideshow.touchFirstPosition = null;
                        }
                    ...
                    pgwSlideshow.plugin.find('.ps-current').on('touchmove', function(e) {
                        try {
                            if (e.touches[0].clientX && pgwSlideshow.touchFirstPosition != null) {
                                if (e.touches[0].clientX > (pgwSlideshow.touchFirstPosition + 50)) {
                                    pgwSlideshow.touchFirstPosition = null;
                                    pgwSlideshow.previousSlide();
                                } else if (e.touches[0].clientX < (pgwSlideshow.touchFirstPosition - 50)) {
                                    pgwSlideshow.touchFirstPosition = null;
                                    pgwSlideshow.nextSlide();
                                }
                            }
                    ...
                    pgwSlideshow.plugin.find('.ps-list > ul').on('touchmove', function(e) {
                        try {
                            if (e.touches[0].clientX) {
                                ...
                                nbPixels = (pgwSlideshow.touchListLastPosition == false ? 1 : Math.abs(lastPosition - e.touches[0].clientX));
                                pgwSlideshow.touchListLastPosition = e.touches[0].clientX;

                                var touchDirection = '';
                                if (lastPosition > e.touches[0].clientX) {
                                    touchDirection = 'left';
                                } else if (lastPosition < e.touches[0].clientX) {
